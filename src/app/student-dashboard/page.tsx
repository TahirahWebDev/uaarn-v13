...
export default function StudentDashboard() {
  ...
  if (!user) redirect(`/sign-in?redirect_url=/student-dashboard`);

  const cards = [
    { href: "/courses", icon: BookOpen, title: "View Courses", description: "Browse, enroll, and continue your high-fidelity learning journey.", accent: "#2B7574", delay: 0.15 },
    { href: "/student-dashboard/uploaded-notes", icon: FileText, title: "My Notes", description: "Access and download neural assets shared by your architects.", accent: "#0E2931", delay: 0.25 },
    { href: "/quiz", icon: Brain, title: "Generate Quiz", description: "Initialize AI-powered assessments based on course materials.", accent: "#861211", delay: 0.35 },
  ];
        ...
        <div className="bg-white rounded-[2.5rem] w-full overflow-hidden flex flex-col md:flex-row min-h-[750px] shadow-2xl">
          ...
          <div className="w-full md:w-[40%] relative bg-[#0E2931] text-[#E2E2E0] p-12 md:p-16 flex flex-col justify-between overflow-hidden">
              ...
              <SlideIn direction="left">
                ...
                <h1 className="text-5xl font-black uppercase tracking-tighter leading-[0.9] mb-8">
                  ...
                  <span className="text-white drop-shadow-[2px_2px_2px_rgba(134,18,17,0.8)] inline-block">
                    <TypewriterText text={user.firstName || "Student"} loop={false} cursor={false} />
                  </span>
                </h1>
            ...
            <div className="relative z-10 space-y-2 opacity-40 pt-12">
              <motion.div animate={{ opacity: [0.4, 0.7, 0.4] }} transition={{ duration: 2, repeat: Infinity }} className="text-[10px] font-black uppercase tracking-[0.5em]">Neural Link: Stable</motion.div>
          ...
          <div className="w-full md:w-[60%] bg-[#E2E2E0] p-10 md:p-14 flex flex-col justify-center">
            <FadeIn>
              <div className="flex items-center gap-4 mb-12">
                <h3 className="text-xs font-black text-[#0E2931] uppercase tracking-[0.3em] whitespace-nowrap">Learning Operations</h3>
                <motion.div
            ...
            <div className="flex flex-col gap-6">
              {cards.map(({ href, icon: Icon, title, description, accent, delay }) => (
                <motion.div
                  key={href}
                ...
                >
                  <Link href={href}>
                    <motion.div
                      ...
                      className="group bg-white rounded-[2rem] p-8 flex items-center justify-between border-2 border-transparent hover:shadow-2xl transition-shadow duration-300 shadow-lg shadow-[#0E2931]/5"
                      style={{ ["--accent" as string]: accent }}
                    >
                      <div className="flex items-center gap-8">
                        <motion.div
                          className="w-16 h-16 flex-shrink-0 rounded-2xl flex items-center justify-center text-white shadow-xl transition-colors duration-500"
                          style={{ backgroundColor: "#0E2931" }}
                          whileHover={{ backgroundColor: accent, rotate: -6 }}
                          transition={{ duration: 0.3 }}
                        >
                          <Icon size={24} />
                        </motion.div>
                        <div>
                          <h2 className="text-xl font-black text-[#0E2931] uppercase tracking-tighter mb-1">{title}</h2>
                      ...
                      <motion.div
                        ...
                        whileHover={{ scale: 1.15, color: accent, borderColor: accent }}
